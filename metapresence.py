...
parser.add_argument("-o",metavar='output_prefix',default='metapout',help="the prefix of the .tsv output file. Default: metapout")
        ...
        for contig in genome_contigs[seq]:
            ...
            try:
                for line in sam.fetch(contig=contig):
                    rn+=1
                    a=str(line).split('\t')                 
                    if a[0] not in pair_taken:
                        allpositions.append(int(a[3])+genome_pointer)
                        pair_taken[a[0]]=1                                              
                    else:                                                                   
                        allpositions2.append(int(a[3])+genome_pointer)                            
                    start_pos=int(a[3]) 
                    end_pos=start_pos+len(a[9])
