...
parser.add_argument("--quiet",help='set this flag to suppress warning messages. default: FALSE',action="store_true")
parser.add_argument('--version', '-v', '-version', action='version', version='metapresence 1.0')

args=parser.parse_args()

if not os.path.isdir(args.input_fasta) and not args.input_bins:
    print('\nError: if < input_fasta > is not a folder of fasta files, < -input_bins > must be specified. \n')
    exit(1)
...
if __name__ == "__main__":  
    ...
    if os.path.isdir(scaffold_input):
        ...
        contigs_lengths_d,genome_contigs=parse_SingleFasta(scaffold_input,inputbin)
    else:
