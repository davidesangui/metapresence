...
if len(sys.argv)==1:
    print('usage: bowtie2 alignment launch ! [options] -s scaffolds_to_bin.txt all_sequences.fasta')
...
if len(sys.argv)==2 and sys.argv[1]=='-h':
    ...
    print('     -s A file with each line listing a scaffold and a bin name, tab-seperated')
...
def multi_main(array,processori):
    ...
    for i in range(len(blocchi)):        
        j=multiprocessing.Process(target=main,args=(blocchi[i],i,bowtie2_launch,contigs_lengths_d,genome_contigs))
        jobs.append(j)
    for j in jobs:        
        j.start()
    ...
    for seq in list_of_genomes:
        ...
        for contig in genome_contigs[seq]:
            ...
            if len(positions)<3: # if less than three fragments map on the contig, it will count in the wheighted average according to the penalty (option)
                ...
                print(contig+'\t'+str(contig_lengths_d[contig])+'\t'+str(total_depth/contig_lengths_d[contig])+'\t'+str(zerodepth/contig_lengths_d[contig])+'\t'+str((total_depth/contig_lengths_d[contig])/(1-np.exp(-0.883*(total_depth/contig_lengths_d[contig]))))+'\t'+'0.5'+'\t'+'0.5'+'\t'+str(rn),file=zz)        
                continue 
